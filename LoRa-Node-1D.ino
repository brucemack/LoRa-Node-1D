...
int counter = 0;
//uint8_t targetNodeAddr = 4;
uint8_t targetNodeAddr = 3;
...
void send_1() {
  ...
  uint8_t rc = mesh_manager.sendtoWait(data, data_len, targetNodeAddr);
      ...
      if (mesh_manager.recvfromAckTimeout(rec_data, &rec_len, rec_timeout, &rec_source)) {
        ...
        if (rec_data[1] == 2) {
          if (rec_len >= 40) {
            ...
            Serial.print(", \"msg\": \"");
            Serial.print((const char*) & (rec_data[24]));
            Serial.println("\" }");
